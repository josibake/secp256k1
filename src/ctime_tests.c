...
static void run_tests(secp256k1_context *ctx, unsigned char *key) {
    ...
    unsigned char prevout_summary_ser[33] = { 0 };
    ...
    ret = secp256k1_silentpayments_recipient_prevout_summary_create(ctx, &prevout_summary, outpoint_smallest, xonly_pubkeys, 1, plain_pubkeys, 1);
    CHECK(ret == 1);
     ...
     */
    CHECK(secp256k1_silentpayments_recipient_scan_outputs(ctx, found_outputs, &n_found_outputs, tx_outputs, 1, key, &prevout_summary, &recipient.spend_pubkey, NULL, NULL));
     ...
     */
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_serialize(ctx, prevout_summary_ser, &prevout_summary));
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_parse(ctx, &prevout_summary, prevout_summary_ser));
    CHECK(secp256k1_silentpayments_recipient_create_shared_secret(ctx, shared_secret, key, &prevout_summary));
