...
int secp256k1_silentpayments_sender_create_outputs(const secp256k1_context *ctx, secp256k1_xonly_pubkey **generated_outputs, const secp256k1_silentpayments_recipient **recipients, size_t n_recipients,
        const unsigned char *outpoint_smallest36, const secp256k1_keypair * const *taproot_seckeys, size_t n_taproot_seckeys, const unsigned char * const *plain_seckeys, size_t n_plain_seckeys
) {
    size_t i, k;
    ...
    for (i = 0; i < n_recipients; i++) {
        if (recipients[i]->index != i) {
            printf("something is wrong with the indexes, got %lu, expected %lu\n", recipients[i]->index, i);
            return 0;
        }
    }
...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    if (!secp256k1_silentpayments_create_shared_secret(ctx, shared_secret, public_tweak_data, scan_key, (input_hash != NULL) ? input_hash : NULL)) {
