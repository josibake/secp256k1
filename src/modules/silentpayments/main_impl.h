 ...
 *    prevout_summary object and always deserializes into a prevout_summary object with combined = true
 *    (and the input_hash portion zeroed out).
 */

int secp256k1_silentpayments_recipient_prevout_summary_create(
    const secp256k1_context *ctx,
    ...
    /* Sanity check inputs */
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(prevout_summary != NULL);
    ARG_CHECK(outpoint_smallest36 != NULL);
    ...
    VERIFY_CHECK(!overflow);
    memcpy(&prevout_summary->data[0], secp256k1_silentpayments_prevout_summary_magic, 4);
    prevout_summary->data[4] = 0;
    secp256k1_ge_to_bytes(&prevout_summary->data[5], &input_pubkey_sum_ge);
    memcpy(&prevout_summary->data[5 + pubkeylen], input_hash_local, 32);
    return 1;
}

int secp256k1_silentpayments_recipient_prevout_summary_serialize(const secp256k1_context *ctx, unsigned char *output33, const secp256k1_silentpayments_recipient_prevout_summary *prevout_summary) {
    secp256k1_ge ge;
    size_t pubkeylen = 33;
    int ret, combined;
    ...
    ARG_CHECK(prevout_summary != NULL);
    ARG_CHECK(secp256k1_memcmp_var(&prevout_summary->data[0], secp256k1_silentpayments_prevout_summary_magic, 4) == 0);
     ...
     *   - `_tweak_mul` can only fail if input_hash_scalar is zero, but assuming the prevout_summary object
     ...
     *   prevout_summary object is created.
     */
    secp256k1_ge_from_bytes(&ge, &prevout_summary->data[5]);
    combined = (int)prevout_summary->data[4];
    ret = 1;
    if (!combined) {
        secp256k1_scalar input_hash_scalar;
        secp256k1_scalar_set_b32(&input_hash_scalar, &prevout_summary->data[5 + 64], NULL);
...
int secp256k1_silentpayments_recipient_prevout_summary_parse(const secp256k1_context *ctx, secp256k1_silentpayments_recipient_prevout_summary *prevout_summary, const unsigned char *input33) {
    size_t inputlen = 33;
    secp256k1_ge pk;
     ...
     */
    memcpy(&prevout_summary->data[0], secp256k1_silentpayments_prevout_summary_magic, 4);
    prevout_summary->data[4] = 1;
    secp256k1_ge_to_bytes(&prevout_summary->data[5], &pk);
    memset(&prevout_summary->data[5 + 64], 0, 32);
    return 1;
}
...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    secp256k1_ge_from_bytes(&input_pubkey_sum_ge, &prevout_summary->data[5]);
    combined = (int)prevout_summary->data[4];
    if (!combined) {
        secp256k1_scalar input_hash_scalar;
        secp256k1_scalar_set_b32(&input_hash_scalar, &prevout_summary->data[5 + 64], NULL);
...
int secp256k1_silentpayments_recipient_create_shared_secret(const secp256k1_context *ctx, unsigned char *shared_secret33, const unsigned char *recipient_scan_key32, const secp256k1_silentpayments_recipient_prevout_summary *prevout_summary) {
    ...
    secp256k1_ge_from_bytes(&input_pubkey_ge, &prevout_summary->data[5]);
    combined = (int)prevout_summary->data[4];
    if (!combined) {
        secp256k1_scalar input_hash_scalar;
        secp256k1_scalar_set_b32(&input_hash_scalar, &prevout_summary->data[5 + 64], NULL);
