...
static int secp256k1_silentpayments_ecdh_return_pubkey(unsigned char *output, const unsigned char *x32, const unsigned char *y32, void *data) {
     ...
     *
     * Worth mentioning: this function should never fail since it is only called from secp256k1_ecdh,
     * where a point is generated and the x and y value are serialized as bytes. This function takes those
     * bytes and turns them back into a point so that we can serialize the point as a compressed public key.
     * This compressed serialization will be hashed in a later step before being used as the shared secret.
     */
    ret &= secp256k1_fe_set_b32_limit(&x, x32);
    ret &= secp256k1_fe_set_b32_limit(&y, y32);
    secp256k1_ge_set_xy(&point, &x, &y);

    /* Serialize as compressed pubkey */
    ret &= secp256k1_eckey_pubkey_serialize(&point, output, &ser_size, 1);
    return ret;
}
...
static int secp256k1_silentpayments_create_shared_secret(const secp256k1_context *ctx, unsigned char *shared_secret33, const unsigned char *secret_component, const secp256k1_pubkey *public_component, const unsigned char *input_hash) {
    ...
    ret = 1;
    if (input_hash != NULL) {
        ret &= secp256k1_ec_seckey_tweak_mul(ctx, tweaked_secret_component, input_hash);
    }

    /* Compute shared_secret = tweaked_secret_component * Public_component */
    ret &= secp256k1_ecdh(ctx, shared_secret33, public_component, tweaked_secret_component, secp256k1_silentpayments_ecdh_return_pubkey, NULL);
    return ret;
}
...
int secp256k1_silentpayments_sender_create_outputs(
    ...
    for (i = 0; i < n_recipients; i++) {
        if ((secp256k1_ec_pubkey_cmp(ctx, &last_recipient.scan_pubkey, &recipients[i]->scan_pubkey) != 0) || (i == 0)) {
            ...
            ret &= secp256k1_silentpayments_create_shared_secret(ctx, shared_secret, a_sum, &recipients[i]->scan_pubkey, input_hash);
            k = 0;
        }
...
int secp256k1_silentpayments_recipient_public_data_serialize(const secp256k1_context *ctx, unsigned char *output33, const secp256k1_silentpayments_public_data *public_data) {
    ...
    ret = secp256k1_silentpayments_recipient_public_data_load(ctx, &pubkey, input_hash, public_data);
...
int secp256k1_silentpayments_recipient_scan_outputs(
    ...
    combined = (int)public_data->data[0];
    {
        ...
        /* These functions will only fail if there is something malformed about the public keys or public_data, but it
         * is very unlikely we would ever reach this point with malformed data since the caller would have encountered
         * an error when constructing the public key objects and public data object. Given that, we prefer to accumulate
         * the errors instead of trying to fail early to save the caller from doing extra work.
         */
        ret &= secp256k1_silentpayments_recipient_public_data_load(ctx, &A_sum, input_hash_ptr, public_data);
        secp256k1_pubkey_load(ctx, &recipient_spend_pubkey_ge, recipient_spend_pubkey);
        ret &= secp256k1_silentpayments_create_shared_secret(ctx, shared_secret, recipient_scan_key, &A_sum, input_hash_ptr);
    }
...
int secp256k1_silentpayments_recipient_create_shared_secret(const secp256k1_context *ctx, unsigned char *shared_secret33, const unsigned char *recipient_scan_key, const secp256k1_silentpayments_public_data *public_data) {
    ...
    ret = secp256k1_silentpayments_recipient_public_data_load(ctx, &A_tweaked, NULL, public_data);
    ret &= secp256k1_silentpayments_create_shared_secret(ctx, shared_secret33, recipient_scan_key, &A_tweaked, NULL);
    return ret;
}
