...
static void bench_silentpayments_output_scan(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(secp256k1_silentpayments_recipient_prevout_summary_parse(data->ctx, &prevout_summary, data->input_pubkey33));
...
static void bench_silentpayments_full_tx_scan(void* arg, int iters, int use_labels) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(secp256k1_silentpayments_recipient_prevout_summary_create(data->ctx,
            &prevout_summary,
