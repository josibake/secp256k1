...
static void test_recipient_api(void) {
    secp256k1_silentpayments_recipient_prevout_summary pd; /* public data */
    secp256k1_silentpayments_recipient_prevout_summary md; /* malformed public data */
    ...
    pp[0] = &p;
    fp[0] = &f;
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, pp, 1));
     ...
     */
    pp[0] = &malformed_p;
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, pp, 1));
    pp[0] = &p;
    ...
    /* Check null values are handled */
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, NULL, SMALLEST_OUTPOINT, tp, 1, pp, 1));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, NULL, tp, 1, pp, 1));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 1, pp, 1));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, NULL, 1));

    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_serialize(CTX, NULL, &pd));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_serialize(CTX, o, NULL));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_parse(CTX, NULL, o));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_parse(CTX, &pd, NULL));

    /* Check that serializing or using a malformed, i.e., not created by us, public data object fails */
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_serialize(CTX, o, &md));
    ...
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_parse(CTX, &pd, malformed) == 0);

    /* This prevout_summary object was created with combined = 0, i.e., it has both the input hash and summed public keypair. */
    CHECK(secp256k1_silentpayments_recipient_create_shared_secret(CTX, o, ALICE_SECKEY, &pd));

    /* Parse a prevout_summary object from a 33 byte serialization and check that this round trips into a valid serialization */
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_parse(CTX, &pd, BOB_ADDRESS[0]));
    CHECK(secp256k1_silentpayments_recipient_prevout_summary_serialize(CTX, o, &pd));
    ...
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 0, pp, 1));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, tp, 1, pp, 0));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 0, pp, 0));
    CHECK_ILLEGAL(CTX, secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &pd, SMALLEST_OUTPOINT, NULL, 0, NULL, 0));
...
void run_silentpayments_test_vector_receive(const struct bip352_test_vector *test) {
    ...
    secp256k1_silentpayments_recipient_prevout_summary prevout_summary, prevout_summary_index;
    ...
    /* prepare the inputs */
    {
        ...
        ret = secp256k1_silentpayments_recipient_prevout_summary_create(CTX, &prevout_summary,
            test->outpoint_smallest,
        ...
        /* If we are unable to create the prevout_summary object, e.g., the input public keys sum to
    ...
    if (test->num_labels == 0) {
        CHECK(secp256k1_silentpayments_recipient_prevout_summary_serialize(CTX, light_client_data, &prevout_summary));
        CHECK(secp256k1_silentpayments_recipient_prevout_summary_parse(CTX, &prevout_summary_index, light_client_data));
        CHECK(secp256k1_silentpayments_recipient_create_shared_secret(CTX, shared_secret_lightclient, test->scan_seckey, &prevout_summary_index));
        n_found = 0;
        {
            int found = 0;
            size_t k = 0;
