...
int main(void) {
    ...
    /*** Receiving ***/
    {
...

        {
             ...
             *        and call `_silentpayments_recipient_prevout_summary_create`
             ...
             * creates a `secp256k1_silentpayments_recipient_prevout_summary` object. He uses
             * this for his own scanning and also serializes the `prevout_summary`
             * object to send to light clients. We will use this later for
             * Carol, who is scanning as a light client. Note, anyone can create
             * and provide these `prevout_summary` objects, i.e. you don't need to be
             ...
             * with light clients.
             */
            ret = secp256k1_silentpayments_recipient_prevout_summary_create(ctx,
                &prevout_summary,
                smallest_outpoint,
            ...
            if (!ret) {
                 ...
                 * could cause our node to crash if, e.g., we assume that prevout_summary_create
                 * will always succeed."
                 */
                printf("\n");
            ...
            ret = secp256k1_silentpayments_recipient_prevout_summary_serialize(ctx,
                light_client_data33,
                &prevout_summary
            );
            assert(ret);
        ...
        }
        {
             ...
             * `prevout_summary` object created by Bob's full node earlier. This
             * serialized `prevout_summary` object contains everything she needs for
            ...
            /* Scan one output at a time, using the serialized `prevout_summary`
             * created by Bob's full node
             */
            ret = secp256k1_silentpayments_recipient_prevout_summary_parse(ctx,
                &prevout_summary,
                light_client_data33
            );
            if (!ret) {
