...
/* Use secp256k1_memcmp_var instead of memcmp.
 *
 * Normally, memcmp should be fine, but we use secp256k1_memcmp_var
 ...
 */
#define memcmp secp256k1_memcmp_var
...
int main(void) {
    ...
    /*** Receiving ***/
    {
...

        {
            ...
            secp256k1_pubkey unlabelled_spend_pubkey;
            secp256k1_pubkey labelled_spend_pubkey;
            secp256k1_pubkey address_labelled_spend_pubkey;
            ...
            }
            {
                 ...
                 */
                ret = secp256k1_silentpayments_recipient_create_labelled_spend_pubkey(ctx,
                    &labelled_spend_pubkey,
                    &unlabelled_spend_pubkey,
                    &label
                );
                assert(ret);
                /* Load the labelled spend public key from Bob's address */
                ret = secp256k1_ec_pubkey_parse(ctx,
                    &address_labelled_spend_pubkey,
                    bob_address[1],
                    33
                );
                assert(ret);
                if (secp256k1_ec_pubkey_cmp(ctx, &labelled_spend_pubkey, &address_labelled_spend_pubkey) != 0) {
                    printf("\n");
                    printf("Something went wrong, the labelled spend public key does not match Bob's address.");
                };
            }
            ...
            if (n_found_outputs > 0) {
                ...
                for (i = 0; i < n_found_outputs; i++) {
                     ...
                     * If the output was sent to a labelled address, the label tweak has
