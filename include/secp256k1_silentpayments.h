 ...
 *
 *  `outpoint_smallest36` refers to the smallest outpoint lexicographically from the transaction
 *  inputs (both silent payments eligible and non-eligible inputs). This value MUST be the smallest
 *  outpoint out of all of the transaction inputs, otherwise the recipient will be unable to find the the payment.
 *
 ...
 *         outpoint_smallest36: serialized smallest outpoint (lexicographically) from the transaction inputs
 ...
 *  eligible input to spend) and a serialized outpoint_smallest, create a `public_data` object.
 *  This object summarizes the public data from the transaction inputs needed for scanning.
 *
 *  `outpoint_smallest36` refers to the smallest outpoint lexicographically from the transaction
 *  inputs (both silent payments eligible and non-eligible inputs). This value MUST be the smallest
 *  outpoint out of all of the transaction inputs, otherwise the recipient will be unable to find the the payment.
 ...
 *  in an index for later use (e.g. wallet rescanning, vending data to light clients).
 *
 ...
 *                              input_hash.
 *  In:    outpoint_smallest36: serialized smallest outpoint (lexicographically) from the transaction inputs
 ...
 *
 *  This function is implemented by the recipient to check if a value exists in the recipients label cache during scanning.
 *
 *  Returns: pointer to the 32-byte label tweak if there is a match.
 *           NULL pointer if there is no match.
 *
 *  In:         label: pointer to the label value to check (computed during scanning)
 *      label_context: pointer to the recipients label cache.
 */
typedef const unsigned char* (*secp256k1_silentpayments_label_lookup)(const secp256k1_pubkey* label, const void* label_context);
 ...
 *  found_with_label: boolean value to indicate if the output was sent to a labelled address. If true,
 *                    label will be set with a valid public key.
 *             label: public key representing the label used. If found_with_label = false, this is set
 ...
 *
 *  Given the public input data (secp256k1_silentpayments_public_data), calculate the shared secret.
 *
 *  The resulting shared secret is needed as input for creating silent payments
 *  outputs belonging to the same recipient scan public key. This function is intended for light clients,
 *  i.e., scenarios where the caller does not have access to the full transaction. If the caller
 *  does have access to the full transaction, `secp256k1_silentpayments_recipient_scan_outputs`
